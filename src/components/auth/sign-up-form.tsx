...
export function SignUpForm() {
  ...
  const translationsData = {
    en: {
      ...
      errorConfig: "Registration not configured.",
    ...
    fr: {
      ...
      errorInUse: "Cet e-mail est déjà enregistré.",
      errorWeak: "Minimum 6 caractères.",
      errorConfig: "Service non actif.",
    ...
    ar: {
      ...
      errorInUse: "هذا البريد مسجل بالفعل.",
      errorWeak: "يجب أن تكون 6 أحرف على الأقل.",
    ...
    try {
      ...
      if (err instanceof Error && 'code' in err) {
        ...
        if (errorCode === 'auth/email-already-in-use') {
           setError(translations.errorInUse);
        } else if (errorCode === 'auth/weak-password') {
           setError(translations.errorWeak);
      ...
      >
        ...
        <form onSubmit={handleSubmit} className="space-y-10 group/form">
          ...
          <div className="flex justify-between items-center pt-8 border-t border-white/20">
            <span className="text-[11px] uppercase tracking-widest !text-white/50 mt-1 font-mono py-2">
              S-ARCH
            </span>
            <Link 
              href={`/${locale}/auth/signin`} 
              className="text-[11px] uppercase tracking-widest !text-white/60 hover:!text-white transition-colors duration-500 font-mono group/link relative py-2"
            >
              {translations.loginLink}
              <div className="absolute bottom-0 right-0 h-px w-0 bg-white group-hover/link:w-full transition-all duration-500" />
            </Link>
          </div>
        </form>
      </motion.div>
